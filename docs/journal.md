...
# Homelab Service Health Check – May 24, 2025

## Wins:
- Successfully configured a Python-based health check script using `Rich` and `Typer` for CLI rendering.
- Wired the health check into the `Makefile` via `make health`.
- Integrated a structured JSON log system that appends timestamped service statuses after each check.
- Added custom DNS mappings (`.local`) into Windows `hosts` file via batch script and confirmed proxy resolution works.
- Ensured Jellyfin and Home Assistant were reachable both by IP and `.local` hostnames.
- Resolved persistent container mapping issues with `jellyfin` by verifying volume paths and network consistency.

## Losses:
- Attempted to build a dashboard using `matplotlib` for visualizing health check data.
- Encountered a limitation with `FigureCanvasAgg` in non-GUI WSL environments.
- Determined that using `VcXsrv` or X11 forwarding introduced unnecessary security risks and overhead.
- Ultimately decided to remove the dashboard renderer for a more lightweight and secure setup.

## What’s Next:
- Optionally explore scheduled automation (cron or `schtasks`) for the health check.
- Expand the health check script to optionally log in `.txt` or `.csv` format.
- Consider implementing email or Discord webhook alerts if a service is reported "Down".
- Build a shared `proxy-config-note.md` to track mappings, aliases, and domain routing strategy.

## Lessons Learned:
- Always validate Docker Compose volume paths when using WSL; absolute paths (`/mnt/c/...`) work best.
- Proxy domain resolution under Windows requires elevated Notepad edits to `C:\Windows\System32\drivers\etc\hosts`.
- Aliases in Docker Compose external networks are not supported in v3 syntax—these must be managed elsewhere.
- Sometimes a clean CLI utility beats fancy dashboards, especially in constrained or cross-platform setups.
